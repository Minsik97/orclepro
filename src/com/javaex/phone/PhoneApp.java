...
public class PhoneApp {
	...
	public static void main(String[] args) {
		...
		/*
		 * 테스트용 데이터 넣기 PhoneVo phoneVo01 = new PhoneVo ("이효리", "010-1111-1111",
		 * "02-1111-1111"); phoneDao.personInsert(phoneVo01);
		 * 
		...
		/*
		 * 테스트용 업데이트 PhoneVo pupVo = new PhoneVo(5, "서장훈", "010-5555-5555",
		 * "02-5555-5555"); phoneDao.personUpdate(pupVo);
		 */

		/*
		 * 테스트용 삭제기능 phoneDao.phoneDelete(1);
		 */

		/*
		 * 테스트용 리스트 출력 phoneList = phoneDao.getPersonList(); for(int i=0; i <
		 * phoneList.size(); i++) { PhoneVo vo = phoneList.get(i);
		 * System.out.println(vo.getPhoneId() + ", " + vo.getName()+ ", " + vo.getHp() +
		 * ", " + vo.getCompany()); }
		 */
		...
		while (true) {
			...
			if (num == 6) {
				...
				for (int i = 0; i < phoneList.size(); i++) {
					...
					System.out.println(
							vo.getPhoneId() + ", " + vo.getName() + ", " + vo.getHp() + ", " + vo.getCompany());
				}
				...
				if (num3.length() > 0) {
					phoneList = phoneDao.psList(num3);

					for (int i = 0; i < phoneList.size(); i++) {
						PhoneVo ps = phoneList.get(i);
						System.out.println(ps.getPhoneId() + ", " + ps.getName() + ", " + ps.getHp() + ", " + ps.getCompany());
					}
					System.out.println("");
					sc.nextLine();
				}
